...
const AuthProvider = ({ children }: AuthProviderProps) => {
  ...
  const login = async (data: LoginAxiosRequest): Promise<void> => {
    ...
    try {
      const response: AxiosResponse<LoginAxiosResponse> = await api.post<LoginAxiosResponse>(
        `${authUri}/login`,
        data,
      );
  ...
  const refreshToken = useCallback(async (): Promise<void> => {
    try {
      const response = await api.post<LoginAxiosResponse>(`${config.authUri}/refreshToken`);
  ...
  return (
    <AuthContext.Provider value={value}>
      {children}
      {popupMessage && <Popup message={popupMessage} onClose={() => setPopup('')} />}
